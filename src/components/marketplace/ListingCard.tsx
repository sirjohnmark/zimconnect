...
function ImageFallback() {
  return (
    <div className="flex h-full w-full items-center justify-center bg-gray-100">
      <svg className="h-10 w-10 text-gray-300" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24" aria-hidden="true">
...
export function ListingCard({ listing, href, imageAspect = "4/3", className }: ListingCardProps) {
  ...
  return (
    <div className={cn(
      "group flex flex-col rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden",
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-1.5 p-4">
          ...
          <div className="flex items-center justify-between gap-2 mt-auto pt-1.5">
            ...
            {category && (
              <span className="text-xs text-gray-400 truncate shrink-0">{category.name}</span>
            )}
          </div>
        </div>
      </Link>
