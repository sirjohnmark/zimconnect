...
export interface BuyerDashboard {
  user: AuthUser;
  default_dashboard: "buyer" | "seller";
  can_apply_to_sell: boolean;
...
export async function getBuyerDashboard(): Promise<BuyerDashboard> {
  if (USE_MOCK) {
    ...
    return {
      user: stored as unknown as AuthUser,
      default_dashboard: "buyer",
      can_apply_to_sell: true,
      seller_application: null,
      saved_listings_count: getSavedIds().length,
      conversations_count: 0,
    };
  }
  return api.get<BuyerDashboard>("/api/v1/buyers/dashboard");
}
