...
function NavLink({
  ...
  // Exact-match for dashboard roots to avoid marking parent active on sub-pages
  const exactMatchHrefs = ["/dashboard", "/dashboard/buyer", "/dashboard/seller"];
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-5 overflow-y-auto px-3 py-5">
        <NavSection title="Marketplace">
          ...
          <NavLink
            item={{ ...MARKETPLACE_NAV[0], href: homeHref }}
            onClick={onNavClick}
          />
          {/* Messages */}
          <NavLink
            item={MARKETPLACE_NAV[1]}
            badge={unreadMessages}
            onClick={onNavClick}
          />
          {/* My Listings — SELLER gets seller listings page; ADMIN keeps the admin listings path */}
          {(isSeller || isAdmin) && (
            <NavLink
              item={{
                ...MARKETPLACE_NAV[2],
                href: isSeller ? "/dashboard/seller/listings" : "/dashboard/listings",
              }}
              onClick={onNavClick}
            />
          )}
          {/* Orders and Saved — visible to everyone */}
          {MARKETPLACE_NAV.slice(3).map((item) => (
