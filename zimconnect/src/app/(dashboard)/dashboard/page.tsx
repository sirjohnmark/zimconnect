...
export default function DashboardPage() {
  ...
  // Route BUYER and SELLER to their dedicated dashboards.
  // ADMIN/MODERATOR stay on this page (general overview).
  useEffect(() => {
  ...
  useEffect(() => {
    if (!user) return;
    // BUYER/SELLER are redirected above; skip data load for them.
  ...
  // Spinner while role-based redirect is in progress
  if (authLoading || user?.role === "BUYER" || user?.role === "SELLER") {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-apple-blue border-t-transparent" />
      </div>
    );
  }
